...
# Installing packages in Sapelo:

```{r}
# shared_lib_path <- "/home/mm60458/instructor_data/shared_R_libs"
                      
#class_packages <- c("tidymodels", "finetune", "vip", "xgboost", "ranger", "tidyverse", "doParallel", "caret", "here", "readr", "dplyr", "kernlab", "lme4")
...
final_spec_xgb <- boost_tree(
  trees = 600,           # Number of boosting rounds (trees)
  tree_depth = 4, # Maximum depth of each tree
  min_n = 10,           # Minimum number of samples to split a node
  learn_rate = 0.0534  # Learning rate (step size shrinkage)
) %>%
  set_engine("xgboost") %>%
  set_mode("regression")
...
write.csv(pred_xgb,here("data", "pred_xgb.csv"),row.names = FALSE)
